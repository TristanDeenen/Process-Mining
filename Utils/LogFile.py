...
class LogFile:
    ...
    def splitTrainTest(self, train_percentage, split_case=True, method="train-test"):
        ...
        if split_case:
            if method == "random":
                train_inds = random.sample(range(self.data.shape[0]), k=round(self.contextdata.shape[0] * train_percentage))
                test_inds = list(set(range(self.data.shape[0])).difference(set(train_inds)))
            elif method == "train-test":
                train_inds = np.arange(0, self.data.shape[0] * train_percentage)
                test_inds = list(set(range(self.data.shape[0])).difference(set(train_inds)))
            else:
                test_inds = np.arange(0, self.data.shape[0] * (1 - train_percentage))
                train_inds = list(set(range(self.data.shape[0])).difference(set(test_inds)))
        else:
            train_inds = []
            test_inds = []
            cases = self.data[self.trace].unique()
            ...
            for train_case in train_cases:
                train_inds.extend(list(self.data[self.data[self.trace] == train_case].index))
            for test_case in test_cases:
                test_inds.extend(list(self.data[self.data[self.trace] == test_case].index))

        train = self.data.loc[train_inds]
        test = self.data.loc[test_inds]
